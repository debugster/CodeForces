...
int main()
{
    ...
    while (t--) {
        ...
        cnt.clear();
        for (auto x : unq) {
            cnt.push_back(make_pair(freq[x], x));
        }
        ...
        if (unq.size() == 1) {
            ...
            cout << "YES\n";
            int root = cnt[0].second;
            int last = cnt[cnt.size() - 1].second;
            int p = where[root][0];
            int c = where[last][0];

            for (auto x : where[last]) {
                cout << p + 1 << " " << x + 1 << endl;
            }
            for (i = 1; i < where[root].size(); i++) {
                cout << c + 1 << " " << where[root][i] + 1 << endl;
            }

            for (i = 1; i < cnt.size() - 1; i++) {
                c = cnt[i].second;
                for (auto x : where[c]) {
                    cout << p + 1 << " " << x + 1 << endl;
                }
